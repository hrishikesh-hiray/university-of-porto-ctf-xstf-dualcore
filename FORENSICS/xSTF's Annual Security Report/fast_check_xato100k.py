...
def ok(pw):
 ...
 return data[:16]==U[:16]
start=time.time();n=0
for line in open('xato-100k.txt',encoding='utf-8',errors='ignore'):
 pw=line.strip('\r\n')
 if not pw: continue
 n+=1
 if ok(pw): print('FOUND',pw,'at',n,'elapsed',time.time()-start); break
else:
 print('no hit',n,'elapsed',time.time()-start)
